...
export default function DayView({ userId }: Props) {
    ...
    const [aiTaskQueue, setAiTaskQueue] = useState<Task[]>([])
    const [aiDurationInput, setAiDurationInput] = useState('')
    const [pendingFromAiQueue, setPendingFromAiQueue] = useState(false)
    ...
    // When AI-generated tasks are queued, prompt for duration before saving.
    useEffect(() => {
        if (pendingNewTask || showAiModal) return
        if (aiTaskQueue.length === 0) return
        const next = aiTaskQueue[0]
        setPendingFromAiQueue(true)
        setPendingNewTask(next)
        setAiDurationInput(formatMinutesToHHMM(next.duration ?? 60))
        setAiSuggestion(`AI created "${next.title}". Confirm duration before saving.`)
        setAiSuggestedDuration(null)
        setAiFamily(null)
        setAiSampleSize(null)
        setAiComputedMedian(null)
        setShowAiModal(true)
    }, [aiTaskQueue, pendingNewTask, showAiModal])
    ...
    const parseDurationStringToMinutes = (s: string, fallback: number) => {
        ...
        if (parts.length === 2) {
            ...
            if (!isNaN(h) && !isNaN(m)) return Math.max(1, h * 60 + Math.min(59, Math.max(0, m)))
        }
    ...
    const acceptAiSuggestion = async () => {
        await persistPendingTask(true)
    }

    const dismissAiSuggestion = async () => {
        await persistPendingTask(false)
    }
    ...
    const runAiCommand = async () => {
        ...
        try {
            ...
            const tasksNeedingTravel: Task[] = []
            const readyTasks: Task[] = []

            for (const t of tasksFromAi) {
                const day = typeof t.day === 'string' && t.day ? t.day : dateKey
                const existing = getTasksForDate(userId, day) ?? []
                const newTask: Task = {
                    ...
                    travelMinutes: undefined,
                    travelMode: (t.travelMode as Task['travelMode']) || taskTravelMode || 'driving',
                }

                if (newTask.location) {
                    tasksNeedingTravel.push(newTask)
                    continue
                }

                readyTasks.push(newTask)
            }

            // If we have tasks with locations, prompt to add travel time (do not auto-save).
            if (tasksNeedingTravel.length > 0) {
                const t = tasksNeedingTravel[0]
                setTaskLocation(t.location || '')
                setTaskTravelMode(t.travelMode || 'driving')
                setPendingAddressTask(t)
                setAddressLookupStatus('Looking up travel time...')
                setAddressLookupMinutes(null)
                setShowAddressModal(true)
                try {
                    const travelRes = await fetchTravelMinutes(t.location!, t.travelMode || 'driving')
                    if (travelRes.minutes != null) {
                        setAddressLookupMinutes(travelRes.minutes)
                        setAddressLookupStatus(`Found ~${travelRes.minutes} minutes`)
                    } else {
                        setAddressLookupMinutes(null)
                        setAddressLookupStatus(travelRes.status || 'No travel time available')
                    }
                } catch (err) {
                    console.warn('AI travel lookup failed', err)
                    setAddressLookupMinutes(null)
                    setAddressLookupStatus('Travel lookup failed; you can still save without travel.')
                }
                // Still add other ready tasks below (without travel)
            }

            // Save ready tasks immediately; surface contextual suggestion in status if available.
            let suggestionMsgs: string[] = []
            for (const task of readyTasks) {
                try {
                    const result = await analyzeEstimate(userId, task.title, task.duration)
                    if (result?.message) {
                        suggestionMsgs.push(`${task.title}: ${result.message}`)
                        if (result.suggestedDuration) {
                            task.duration = result.suggestedDuration
                        }
                    }
                } catch (err) {
                    console.warn('AI contextual suggestion failed', err)
                }

                const day = task.dateKey
                const existing = getTasksForDate(userId, day) ?? []
                const updated = [...existing, task].sort((a, b) => a.time.localeCompare(b.time))
                saveTasksForDate(userId, day, updated)
                if (day === dateKey) {
                    setTasks(updated)
                }
            ...
            if (suggestionMsgs.length > 0) {
                setAiStatus(suggestionMsgs.join(' | '))
            } else if (readyTasks.length === 0 && tasksNeedingTravel.length === 0) {
                setAiStatus('No tasks parsed from AI response.')
            } else {
                setAiStatus('AI tasks added. Travel prompt shown for address tasks.')
            }
        } catch (err) {
            console.error('AI command failed', err)
            setAiStatus('AI command failed (see console).')
        } finally {
            setAiLoading(false)
        }
    }
                        ...
                        <div style={{ position: 'absolute', top: 0, bottom: 0, left: '72px', right: '0px' }}>
                            {lanes.map((lane, laneIndex) => {
                                ...
                                return lane.map(task => {
                                    ...
                                    // Travel block sits immediately before the task window, leaving a small gap to avoid overlap
                                    const travelMins = task.travelMinutes ?? 0
                                    const travelHeightPx = Math.max(travelMins * pxPerMinute, 26)
                                    const travelGapPx = 4
                                    const travelTop = Math.max(0, taskTop - travelHeightPx - travelGapPx)
                    ...
                    <div style={{ backgroundColor: '#0b1222', borderRadius: '12px', padding: '20px', width: '100%', maxWidth: '520px', border: '1px solid #213547', color: '#e6eef8' }}>
                        ...
                        <div style={{ marginTop: 12 }}>
                            ...
                            <input
                                ...
                                inputMode="numeric"
                                pattern="[0-9:]*"
                                value={aiDurationInput}
                                onChange={(e) => {
                                    let raw = e.target.value.replace(/[^0-9:]/g, '').slice(0,5)
                                    if (!raw.includes(':') && raw.length > 2) raw = raw.slice(0,2) + ':' + raw.slice(2)
                                    setAiDurationInput(raw)
                                }}
                                style={{ width: '100%', padding: '10px', borderRadius: 8, border: '1px solid #3f3f46', background: '#0f172a', color: '#e5e7eb' }}
